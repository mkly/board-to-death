...
function signupTokenFromMagicLink(url: string): string | null {
  try {
    const magicLink = new URL(url);
    const callback = new URL(magicLink.searchParams.get("callbackURL") ?? "", magicLink.origin);
    if (callback.pathname !== SIGNUP_CALLBACK_PATH) return null;
    return callback.searchParams.get("intent");
  } catch {
    return null;
  }
}
...
export async function isOrganizationSignupMagicLink(
  database: PrismaClient,
  input: { readonly email: string; readonly url: string },
): Promise<boolean> {
  const token = signupTokenFromMagicLink(input.url);
  if (!token) return false;

  const intent = await database.verification.findFirst({
    where: { identifier: intentIdentifier(token), expiresAt: { gt: new Date() } },
    select: { value: true },
  });
  if (!intent) return false;

  const parsed = storedIntentSchema.safeParse(JSON.parse(intent.value));
  return parsed.success && normalizeEmail(parsed.data.email) === normalizeEmail(input.email);
}
